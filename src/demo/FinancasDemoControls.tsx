...
import { financeScenarioLabels, setFinanceScenario } from '@/fixtures/financialScenarios';
import type { FinanceScenarioKey } from '@/fixtures/types';

interface FinancasDemoControlsProps {
  currentResidenceId: string;
  onSwitchResidence: (id: string) => void;
}

export default function FinancasDemoControls({ currentResidenceId, onSwitchResidence }: FinancasDemoControlsProps) {
  ...
  return (
    <>
      <button
        ...
        aria-label="Cenários financeiros de demonstração"
      >
        ...
        <div className="fixed inset-0 z-[95] flex items-end justify-center">
          ...
          <div className="relative bg-white rounded-t-2xl w-full max-w-app p-5 safe-bottom max-h-[75vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-3">
              <h4 className="text-sm font-semibold text-foreground-800">Demo: cenários financeiros</h4>
              <button
            ...
            <p className="text-xs text-foreground-500 mb-3">
              Selecione um cenário para visualizar como o módulo financeiro se comporta em cada situação.
            </p>
            ...
            <div className="flex flex-wrap gap-2">
              {(Object.entries(financeScenarioLabels) as [FinanceScenarioKey, string][]).map(([key, label]) => (
                <button
                  key={key}
                  onClick={() => {
                    setFinanceScenario(key);
                    setOpen(false);
                  }}
