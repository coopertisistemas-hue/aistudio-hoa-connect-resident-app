...
CREATE TABLE IF NOT EXISTS public.residents (
  ...
  status public.resident_status NOT NULL DEFAULT 'pending',
...
CREATE TABLE IF NOT EXISTS public.resident_staff_notes (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id uuid NOT NULL REFERENCES public.tenants(id) ON DELETE CASCADE,
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT resident_staff_notes_resident_fk FOREIGN KEY (resident_id, tenant_id) REFERENCES public.residents(id, tenant_id) ON DELETE CASCADE
);

-- 4. Create Indexes
CREATE INDEX IF NOT EXISTS idx_residents_tenant_status ON public.residents(tenant_id, status);
CREATE INDEX IF NOT EXISTS idx_residents_profile ON public.residents(profile_id);

CREATE UNIQUE INDEX IF NOT EXISTS residents_tenant_registration_code_uidx
  ON public.residents(tenant_id, registration_code)
  WHERE registration_code IS NOT NULL;

CREATE INDEX IF NOT EXISTS residents_registration_code_trgm_idx
  ON public.residents USING gin (registration_code gin_trgm_ops)
  WHERE registration_code IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_resident_staff_notes_tenant_resident ON public.resident_staff_notes(tenant_id, resident_id);
CREATE INDEX IF NOT EXISTS idx_resident_staff_notes_author ON public.resident_staff_notes(author_profile_id);

-- 5. Helper function: is_active_resident
CREATE OR REPLACE FUNCTION public.is_active_resident(target_tenant_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.residents_protected_fields_immutable()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = ''
AS $$
...
DROP TRIGGER IF EXISTS trg_residents_protected_fields ON public.residents;
CREATE TRIGGER trg_residents_protected_fields
  BEFORE UPDATE ON public.residents
  FOR EACH ROW
  EXECUTE FUNCTION public.residents_protected_fields_immutable();

-- 6.3 Resident status transition validation trigger
CREATE OR REPLACE FUNCTION public.residents_status_transition_check()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = ''
AS $$
  ...
  IF NEW.status IS DISTINCT FROM OLD.status THEN
    ...
    ELSIF OLD.status = 'pending' THEN
      ...
      IF NEW.status = 'active' THEN
        ...
        IF NEW.approved_by_profile_id IS NULL THEN NEW.approved_by_profile_id := public.current_profile_id(); END IF;
      END IF;
...
DROP TRIGGER IF EXISTS trg_residents_status_transition ON public.residents;
CREATE TRIGGER trg_residents_status_transition
  BEFORE UPDATE ON public.residents
  FOR EACH ROW
  EXECUTE FUNCTION public.residents_status_transition_check();

-- 6.4 Append-only triggers for resident_staff_notes
CREATE OR REPLACE FUNCTION public.resident_staff_notes_prevent_mutation()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = ''
AS $$
...
DROP TRIGGER IF EXISTS trg_resident_staff_notes_no_update ON public.resident_staff_notes;
CREATE TRIGGER trg_resident_staff_notes_no_update
  BEFORE UPDATE ON public.resident_staff_notes
  FOR EACH ROW
  EXECUTE FUNCTION public.resident_staff_notes_prevent_mutation();

DROP TRIGGER IF EXISTS trg_resident_staff_notes_no_delete ON public.resident_staff_notes;
CREATE TRIGGER trg_resident_staff_notes_no_delete
  BEFORE DELETE ON public.resident_staff_notes
  FOR EACH ROW
  EXECUTE FUNCTION public.resident_staff_notes_prevent_mutation();
...
ALTER TABLE public.resident_staff_notes ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.resident_staff_notes FORCE ROW LEVEL SECURITY;

-- 7.1 RLS Policies for residents
DROP POLICY IF EXISTS residents_select_policy ON public.residents;
CREATE POLICY residents_select_policy ON public.residents
  FOR SELECT
  TO authenticated
  USING (
    profile_id = public.current_profile_id()
    OR public.has_tenant_permission(tenant_id, 'residents:read')
    OR public.is_platform_admin()
  );

DROP POLICY IF EXISTS residents_insert_policy ON public.residents;
CREATE POLICY residents_insert_policy ON public.residents
  FOR INSERT
  TO authenticated
  WITH CHECK (
    public.has_tenant_permission(tenant_id, 'residents:write')
    AND status = 'pending'
  );

DROP POLICY IF EXISTS residents_update_policy ON public.residents;
CREATE POLICY residents_update_policy ON public.residents
  FOR UPDATE
  TO authenticated
  USING (
    public.has_tenant_permission(tenant_id, 'residents:write')
...
DROP POLICY IF EXISTS resident_staff_notes_select_policy ON public.resident_staff_notes;
CREATE POLICY resident_staff_notes_select_policy ON public.resident_staff_notes
  FOR SELECT
  TO authenticated
  USING (
    public.has_tenant_permission(tenant_id, 'residents:write')
...
DROP POLICY IF EXISTS resident_staff_notes_insert_policy ON public.resident_staff_notes;
CREATE POLICY resident_staff_notes_insert_policy ON public.resident_staff_notes
  FOR INSERT
  TO authenticated
  WITH CHECK (
    public.has_tenant_permission(tenant_id, 'residents:write')
...
GRANT SELECT ON TABLE public.resident_staff_notes TO authenticated;
GRANT EXECUTE ON FUNCTION public.is_active_resident(uuid) TO authenticated;
