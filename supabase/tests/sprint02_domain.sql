...
BEGIN
  ...
  BEGIN
    INSERT INTO public.residence_members (tenant_id, property_id, profile_id, role, status)
  ...
  BEGIN
    INSERT INTO public.residence_members (tenant_id, property_id, profile_id, role, status, start_date, end_date)
  ...
  BEGIN
    INSERT INTO public.residence_members (tenant_id, property_id, profile_id, role, status, start_date)
  ...
  INSERT INTO public.residence_members (tenant_id, property_id, profile_id, role, status, start_date, is_primary)
  ...
  WHERE id = mem_id;
  SELECT * INTO mem_rec FROM public.residence_members WHERE id = mem_id;
  ...
  BEGIN
    UPDATE public.residence_members SET start_date = CURRENT_DATE + 1 WHERE id = mem_id;
  ...
  INSERT INTO public.residence_members (tenant_id, property_id, profile_id, role, status, start_date, end_date, end_reason)
...
BEGIN
  ...
  BEGIN
    INSERT INTO public.household_members (tenant_id, property_id, responsible_profile_id, full_name, relationship, status)
  ...
  INSERT INTO public.household_members (tenant_id, property_id, responsible_profile_id, full_name, relationship, status, start_date)
  ...
  BEGIN
    UPDATE public.household_members SET end_date = start_date - 1 WHERE id = hhm_id;
  ...
  BEGIN
    DELETE FROM public.household_members WHERE id = hhm_id;
  ...
  BEGIN
    UPDATE public.household_members SET tenant_id = tenant_b_id WHERE id = hhm_id;
  ...
  BEGIN
    UPDATE public.household_members SET property_id = 'cccccccc-cccc-cccc-cccc-cccccccccccc'::uuid WHERE id = hhm_id;
  ...
  BEGIN
    UPDATE public.household_members SET responsible_profile_id = profile_2_id WHERE id = hhm_id;
  ...
  UPDATE public.household_members SET status = 'former', end_date = CURRENT_DATE WHERE id = hhm_id;
END $$;
...
BEGIN
  ...
  BEGIN
    SELECT COUNT(*) INTO rec_count FROM public.residence_members;
    IF rec_count <> 0 THEN
  ...
  BEGIN
    SELECT COUNT(*) INTO rec_count FROM public.household_members;
    IF rec_count <> 0 THEN
  ...
  SELECT COUNT(*) INTO rec_count FROM public.residence_members WHERE tenant_id = tenant_a_id;
  IF rec_count <> 0 THEN
  ...
  SELECT COUNT(*) INTO rec_count FROM public.household_members WHERE tenant_id = tenant_a_id;
  IF rec_count <> 0 THEN
...
BEGIN
  ...
  SELECT COUNT(*) INTO rec_count FROM public.residence_members WHERE profile_id = profile_1_id;
  IF rec_count < 1 THEN
  ...
  SELECT COUNT(*) INTO rec_count FROM public.household_members WHERE responsible_profile_id = profile_1_id;
  IF rec_count < 1 THEN
  ...
  SELECT COUNT(*) INTO rec_count FROM public.household_members WHERE property_id = property_a_id;
  IF rec_count < 1 THEN
  ...
  SELECT COUNT(*) INTO rec_count FROM public.household_members WHERE tenant_id = tenant_b_id;
  IF rec_count <> 0 THEN
  ...
  SELECT COUNT(*) INTO rec_count FROM public.residence_members WHERE tenant_id = tenant_b_id;
  IF rec_count <> 0 THEN
...
BEGIN
  ...
  UPDATE public.residence_members SET status = 'revoked', end_date = CURRENT_DATE, end_reason = 'moved_out' WHERE id = mem_id;
  ...
  UPDATE public.residence_members SET status = 'revoked', end_date = CURRENT_DATE, end_reason = 'blocked' WHERE id = mem_id;
  ...
  IF NOT public.is_household_responsible(hhm_ana_id) THEN
  ...
  SELECT COUNT(*) INTO rec_count FROM public.household_members WHERE id = hhm_former_id;
  IF rec_count <> 0 THEN
  ...
  SELECT COUNT(*) INTO rec_count FROM public.household_members WHERE id = hhm_revoked_id;
  IF rec_count <> 0 THEN
  ...
  SELECT COUNT(*) INTO rec_count FROM public.household_members WHERE id = hhm_moved_out_id;
  IF rec_count <> 0 THEN
  ...
  SELECT COUNT(*) INTO rec_count FROM public.household_members WHERE id = hhm_ana_id;
  IF rec_count <> 0 THEN
  ...
  SELECT COUNT(*) INTO rec_count FROM public.household_members WHERE id = crossprop2_hm;
  IF rec_count <> 0 THEN
  ...
  SELECT COUNT(*) INTO rec_count FROM public.household_members WHERE id = hhm_cross_tenant_id;
  IF rec_count <> 0 THEN
...
BEGIN
  ...
  INSERT INTO public.profiles (id, user_id, full_name, status) VALUES (du_p, du_u, 'Dup Test 1', 'active');

  INSERT INTO auth.users (id, instance_id, aud, role, email, encrypted_password, email_confirmed_at)
  VALUES (du2_u, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'dup2.hh@example.com', 'pass', now());
  INSERT INTO public.profiles (id, user_id, full_name, status) VALUES (du2_p, du2_u, 'Dup Test 2', 'active');

  INSERT INTO auth.users (id, instance_id, aud, role, email, encrypted_password, email_confirmed_at)
  VALUES (du3_u, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'dup3.hh@example.com', 'pass', now());
  INSERT INTO public.profiles (id, user_id, full_name, status) VALUES (du3_p, du3_u, 'Dup Test 3', 'active');

  INSERT INTO auth.users (id, instance_id, aud, role, email, encrypted_password, email_confirmed_at)
  VALUES (du4_u, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'dup4.hh@example.com', 'pass', now());
  INSERT INTO public.profiles (id, user_id, full_name, status) VALUES (du4_p, du4_u, 'Dup Test 4', 'active');

  INSERT INTO auth.users (id, instance_id, aud, role, email, encrypted_password, email_confirmed_at)
  VALUES (du5_u, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'dup5.hh@example.com', 'pass', now());
  INSERT INTO public.profiles (id, user_id, full_name, status) VALUES (du5_p, du5_u, 'Dup Test 5', 'active');

  INSERT INTO auth.users (id, instance_id, aud, role, email, encrypted_password, email_confirmed_at)
  VALUES (du6_u, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'dup6.hh@example.com', 'pass', now());
  INSERT INTO public.profiles (id, user_id, full_name, status) VALUES (du6_p, du6_u, 'Dup Test 6', 'active');

  INSERT INTO auth.users (id, instance_id, aud, role, email, encrypted_password, email_confirmed_at)
  VALUES (du7_u, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated', 'dup7.hh@example.com', 'pass', now());
  INSERT INTO public.profiles (id, user_id, full_name, status) VALUES (du7_p, du7_u, 'Dup Test 7', 'active');
  ...
  BEGIN
    INSERT INTO public.household_members (tenant_id, property_id, responsible_profile_id, full_name, relationship, status, linked_profile_id)
  ...
  UPDATE public.residence_members SET status = 'revoked', end_date = CURRENT_DATE, end_reason = 'administrative' WHERE id = du_rm_id;
  ...
  BEGIN
    UPDATE public.household_members SET linked_profile_id = du4_p WHERE id = hhm_id;
    RAISE EXCEPTION 'SPR2-DUP-03 failed: updating household_member to link profile with active residence should be rejected';
  EXCEPTION
    WHEN unique_violation THEN NULL;
  END;

  SELECT COUNT(*) INTO before_count FROM public.household_members WHERE id = hhm_id;
  ...
  SELECT COUNT(*) INTO before_count FROM public.household_members WHERE full_name = 'Failed Insert Test';
  BEGIN
    INSERT INTO public.household_members (tenant_id, property_id, responsible_profile_id, full_name, relationship, status, linked_profile_id)
  ...
  EXCEPTION
    WHEN unique_violation THEN NULL;
  END;
  SELECT COUNT(*) INTO after_count FROM public.household_members WHERE full_name = 'Failed Insert Test';
